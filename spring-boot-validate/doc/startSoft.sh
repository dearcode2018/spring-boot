...
start(){
  ...
  if [[ ${PID_FLAG} -ne 0 ]]; then
    ...
    rm -f ${PID_FILE}
     nohup ${RUN_JAVA} ${JAVA_OPTS} -Dserver.port=${SVC_PORT} -jar ${SVC_DEP_HOME}/${SVC_JAR_NAME} ${PROGRAM_OPTS} > /dev/null 2>&1 &
    echo $! > ${PID_FILE}
